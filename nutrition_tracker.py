...
def create_pdf_summary(totals, targets, selected_foods, user_info):
    ...
    if selected_foods:
        ...
        for item in selected_foods[:20]:  # Limit to prevent overflow
            ...
            p.drawString(50, y_pos, f"• {food['name']}: {servings} serving(s)")
            y_pos -= 15
...
st.session_state.user_units = units
...
with col2:
    uploaded_file = st.file_uploader("Load", type="json", key="upload_progress")
...
if all_inputs.get('weight_kg') and all_inputs.get('activity_level'):
    ...
    st.sidebar.info(
        f"💧 Daily Hydration Goal: {hydration_ml} ml (~{hydration_ml/250:.1f} cups) for {weight_display}"
    )
...
units_display = st.session_state.get('user_units', 'metric')
weight_display = format_weight(final_values['weight_kg'], units_display)
height_display = format_height(final_values['height_cm'], units_display)
...
metrics_config = [
    {
        ...
        'metrics': [
            ...
            ("Current Stats", f"{weight_display} | {height_display}")
        ]
    },
    {
