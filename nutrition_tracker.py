...
- Goal-Specific Caloric Adjustments:
  ...
  - Weight Gain: A controlled 10 percent surplus over TDEE.
...
streamlit run nutrition\_app.py
...
def create_progress_tracking(totals, targets, foods):
 ...
 if deficits:
     ...
     if len(deficit_summary) > 1:
         deficit_list = ", ".join(deficit_summary[:-1])
         summary_text = (
             f"You still need {deficit_list}, and {deficit_summary[-1]}."
         )
     else:
     ...
     if top_suggestions:
         ...
         recommendations.append(smart_pick_text)
         ...
         if len(top_suggestions) > 1:
             ...
             for suggestion in top_suggestions[1:]:
                 ...
                 helped = suggestion['nutrients_helped']
             ...
             recommendations.append(alt_options_text)
     else:
         ...
         if best_food and best_food.get(nutrient, 0) > 0:
             ...
             recommendations.append(alt_option_text)
 ...
 for _, row in df.iterrows():
     ...
     if category in foods:
         foods[category].append({
             ...
             'serving_size_g': row.get('serving_size_g', 100)
         })
 return foods
...
def render_food_item(food, category):
 """Renders a single food item with its controls and mode toggle."""
     ...
     if current_mode == 'servings':
         ...
         with col1:
             ...
             for k in range(1, 6):
                 with button_cols[k - 1]:
                     btn_type = (
                         "primary" if current_serving == float(k)
                         else "secondary"
                     )
                     if st.button(
                         f"{k}", key=f"{key_prefix}_serving_{k}",
                         type=btn_type, help=f"Set to {k} servings",
         ...
         with col2:
             custom_serving = st.number_input(
                 ...
                 key=f"{key_prefix}_custom_serving",
                 label_visibility="collapsed"
             )

     else:
         # Grams mode and calculate per 100g values
         ...
         info_text = (
             f"Per 100g: {cal_per_100g:.0f} kcal | "
             f"{protein_per_100g:.1f}g protein | "
         ...
         if grams_equivalent > 0:
             st.caption(
                 f"💡 {grams_equivalent:.0f}g = "
                 f"{equivalent_servings:.1f} servings"
             )
         ...
         with col1:
             ...
             for i, grams in enumerate(gram_amounts):
                 with button_cols[i]:
                     ...
                     is_current = abs(
                         current_serving - servings_equivalent
                     ) < 0.01
                     btn_type = "primary" if is_current else "secondary"

                     if st.button(
                         f"{grams}g", key=f"{key_prefix}_gram_{grams}",
                         type=btn_type,
                         help=(
                             f"Set to {grams}g "
                             f"({servings_equivalent:.1f} servings)"
                         ),
                         use_container_width=True
                     ):
         ...
         with col2:
             ...
             custom_grams = st.number_input(
                 ...
                 key=f"{key_prefix}_custom_grams",
                 label_visibility="collapsed"
             )
...
weekly_change_kg = targets['estimated_weekly_change']
weekly_change_lbs = kg_to_lbs(weekly_change_kg)
weekly_change_display = (
 f"{weekly_change_lbs:+.2f} lbs" if units_display == 'imperial'
 else f"{weekly_change_kg:+.2f} kg"
)
...
metrics_config = [
 {
     'title': 'Metabolic Information', 'columns': 5, 'metrics': [
         ...
         ("Estimated Weekly Weight Change", weekly_change_display)
     ]
 },
 {
...
st.markdown(
 ...
 "compare to your daily targets. Use the toggle switch (🥄 Servings | "
 "⚖️ 100g) to switch between serving-based and gram-based input modes."
)
...
if selected_foods:
 ...
 with col1:
     if st.button(
         "Download PDF Report 📄", key="export_pdf",
         use_container_width=True
     ):
 ...
 with col2:
     if st.button(
         "Download CSV Data 📊", key="export_csv",
         use_container_width=True
     ):
 ...
 with col3:
     if st.button(
         "Share Progress 📱", key="share_progress", use_container_width=True
     ):
         share_text = f"""
...
Created with the Personal Nutrition Coach.
 ...
 with col1:
     ...
     target_values = [
         targets['protein_g'], targets['carb_g'],
         targets['fat_g'], targets['total_calories']
     ]
 ...
 with col2:
     ...
     if totals['calories'] > 0:
         ...
         st.caption(
             "Please select some foods to see the macronutrient split."
         )
