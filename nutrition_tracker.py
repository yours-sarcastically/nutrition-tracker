...
daily targets based on individual parameters (age, height, weight, activity level)
using the Mifflin-St Jeor equation and evidence-based nutritional guidelines.

The targets are calculated using:
- BMR: Mifflin-St Jeor equation
- TDEE: BMR × Activity multiplier
- Caloric surplus: TDEE + 400 calories for lean bulking
...
def calculate_personalized_targets(age, height_cm, weight_kg, gender='male', 
    ...
    # Calculate protein target (grams and calories)
    protein_grams = protein_per_kg * weight_kg
    protein_calories = protein_grams * 4
    
    # Calculate fat target (calories and grams)
    fat_calories = total_calories * fat_percentage
    fat_grams = fat_calories / 9
    
    # Calculate carbohydrate target (remaining calories)
    ...
    return {
        ...
        'target_weight_gain_per_week': round(weight_kg * 0.0025, 2)  # 0.25% of body weight
    }
...
def load_food_database(file_path):
    ...
    try:
        ...
        # Create a sample database if CSV file is not found
        sample_data = {
            'name': ['Eggs', 'Greek Yogurt', 'Protein Powder', 'Milk', 'Cottage Cheese',
                    'Lentils', 'Chickpeas', 'Olive Oil', 'Peanut Butter', 'Almonds',
                    'Oats', 'White Rice', 'Bananas', 'Pasta', 'Potatoes',
                    'Spinach', 'Broccoli', 'Mixed Vegetables'],
            'serving_unit': ['2 large', '1 cup', '1 scoop', '1 cup', '1 cup',
                           '1 cup cooked', '1 cup cooked', '1 tbsp', '2 tbsp', '1 oz',
                           '1 cup cooked', '1 cup cooked', '1 medium', '1 cup cooked', '1 medium',
                           '1 cup', '1 cup', '1 cup'],
            'calories': [140, 130, 120, 150, 220, 230, 270, 120, 190, 160,
                        150, 205, 105, 220, 160, 7, 25, 25],
            'protein': [12, 20, 25, 8, 25, 18, 15, 0, 8, 6,
                       5, 4, 1, 8, 4, 1, 3, 2],
            'carbs': [1, 9, 2, 12, 8, 40, 45, 0, 8, 6,
                     27, 45, 27, 43, 37, 1, 5, 5],
            'fat': [10, 0, 1, 8, 10, 1, 4, 14, 16, 14,
                   3, 0, 0, 1, 0, 0, 0, 0]
        }
    ...
    # Process all foods from the CSV file
...
if st.button("Calculate Daily Intake", type="primary", use_container_width=True):
    ...
    cal_percent = min(total_calories / targets['total_calories'] * 100, 100) if targets['total_calories'] > 0 else 0
    st.progress(cal_percent / 100, text=f"Calories: {cal_percent:.0f}% of target ({targets['total_calories']} kcal)")

    prot_percent = min(total_protein / targets['protein_grams'] * 100, 100) if targets['protein_grams'] > 0 else 0
    st.progress(prot_percent / 100, text=f"Protein: {prot_percent:.0f}% of target ({targets['protein_grams']}g)")
    ...
    if total_protein < targets['protein_grams']:
        ...
        recommendations.append(f"• You need {deficit:.0f}g more protein for optimal muscle building.")
    ...
    if recommendations:
        ...
        st.success("Outstanding work! You've met all your personalized daily nutritional targets! 🎉")

    # Show surplus/deficit information
    st.subheader("Daily Balance Summary: ⚖️")
    cal_balance = total_calories - targets['tdee']
    if cal_balance > 0:
        st.info(f"📈 You're consuming {cal_balance:.0f} calories above maintenance (surplus for weight gain)")
    else:
        st.warning(f"📉 You're consuming {abs(cal_balance):.0f} calories below maintenance")
...
- **Fat**: 25% of total calories
- **Carbohydrates**: Remaining calories
- **Weight gain target**: 0.25% of body weight per week
""")
