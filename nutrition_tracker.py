...
"""
This script implements an interactive nutrition tracking application based on the
highest standards of evidence-based nutrition science. It supports multi-goal
functionality including weight loss, maintenance, and gain using scientifically
validated calculations and methodologies as outlined in the Evidence-Based
Nutrition Tracker Blueprint.

The application implements all six core principles:
1. BMR calculation using the Mifflin-St Jeor equation
2. TDEE calculation with validated activity multipliers
3. Goal-specific caloric targets using percentage-based approach
4. Protein-first macronutrient architecture
5. Dynamic monitoring with estimated rate of change
6. Fitness integration with resistance training emphasis

Usage:
This Streamlit application requires a CSV file named 'nutrition_results.csv'
containing food data with columns: name, category, serving_unit, calories,
protein, carbs, fat. Run with: streamlit run script_name.py

The application provides personalized nutrition targets based on user inputs
including age, height, weight, sex, activity level, and goal. Users can select
foods and track their daily intake against evidence-based targets.
"""
...
# ------ PRINCIPLE 1 & 2: BMR and TDEE Configuration ------
"""
Scientific Rationale: The Mifflin-St Jeor equation is recognized by the Academy
of Nutrition and Dietetics as the most accurate predictive formula for
estimating BMR in healthy adults. Activity multipliers are based on validated
research for TDEE calculation.
"""
ACTIVITY_MULTIPLIERS = {
    ...
    'extremely_active': 1.9     # Very hard exercise, physical job, or training
                               # twice daily
}

# ------ PRINCIPLE 3: Goal-Specific Caloric Targets ------
"""
Scientific Rationale: Percentage-based adjustments scale the diet's intensity
to the individual's metabolic reality, preventing overly aggressive deficits
for smaller individuals and insufficient surpluses for larger individuals.
"""
GOAL_ADJUSTMENTS = {
    'weight_loss': -0.20,      # -20% from TDEE for effective, sustainable
                               # fat loss
    'maintenance': 0.00,       # 0% from TDEE to maintain current weight
    'weight_gain': 0.10        # +10% over TDEE for muscle growth with minimal
                               # fat gain
}

# ------ PRINCIPLE 4: Protein-First Macronutrient Architecture ------
"""
Scientific Rationale: Protein needs are set first based on body weight and
goal; fat is set to ensure hormonal health; carbohydrates fill remaining
energy needs.
"""
PROTEIN_TARGETS = {
...
FAT_PERCENTAGES = {
    ...
    'weight_gain': 0.25        # 25% of calories to maximize carbs for
                               # performance
}
...
def calculate_bmr(age, height_cm, weight_kg, sex='male'):
    """
    PRINCIPLE 1: Calculate Basal Metabolic Rate Using the Mifflin-St Jeor
    Equation

    Scientific Rationale: The Mifflin-St Jeor equation is the most accurate
    predictive formula for estimating BMR in healthy adults, consistently
    outperforming older equations like the Harris-Benedict.

    Equations:
    - For Men: BMR = (10 × weight in kg) + (6.25 × height in cm) - (5 × age
      in years) + 5
    - For Women: BMR = (10 × weight in kg) + (6.25 × height in cm) - (5 × age
      in years) - 161
...
def calculate_tdee(bmr, activity_level):
    """
    PRINCIPLE 2: Calculate Total Daily Energy Expenditure Based on Activity
    Level

    Scientific Rationale: TDEE represents total "maintenance" calories—the
    energy required to maintain current weight with lifestyle. Calculated by
...
def calculate_goal_specific_calories(tdee, goal):
    """
    PRINCIPLE 3: Calculate Goal-Specific Caloric Targets Using Percentage-Based
    Approach

    Scientific Rationale: Using a percentage of TDEE to set caloric surplus or
    deficit is superior to using a fixed number. A percentage-based adjustment
    scales the diet's intensity to the individual's metabolic reality.
...
def calculate_macronutrient_targets(target_calories, weight_kg, goal):
    """
    PRINCIPLE 4: Calculate Macronutrients Using Protein-First, Goal-Specific
    Strategy

    Scientific Rationale: A "protein-first" approach is the most effective way
    to structure a diet. Protein needs are set first based on body weight and
    goal; fat is set to ensure hormonal health; carbohydrates fill remaining
    energy needs.
...
def calculate_estimated_weekly_change(daily_caloric_adjustment):
    ...
    Scientific Rationale: Calculating and displaying the estimated rate of
    change encourages monitoring of actual weekly weight change. If progress
    stalls or deviates significantly, it is a cue to re-evaluate inputs and
    adjust.
...
def calculate_personalized_targets(age, height_cm, weight_kg, sex='male',
    ...
    This function integrates all six principles from the Evidence-Based
    Nutrition Tracker Blueprint to provide scientifically validated nutritional
    targets.
    ...
    if targets['total_calories'] > 0:
        ...
        targets['protein_percent'] = targets['carb_percent'] = targets[
            'fat_percent'
        ] = 0
...
def get_final_values(user_inputs):
    ...
    for field, value in user_inputs.items():
        if field == 'sex':
            final_values[field] = (
                value if value != "Select Sex" else DEFAULTS[field]
            )
        else:
            final_values[field] = (
                value if value is not None else DEFAULTS[field]
            )
...
print("Initializing Evidence-Based Nutrition Tracker application... 🚀")
foods = load_food_database('nutrition_results.csv')
foods = assign_food_emojis(foods)
print("Food database loaded and emoji rankings assigned successfully! 📊")
...
This application implements the highest standards of evidence-based nutrition
science to provide personalized targets for weight loss, maintenance, or gain.
...
user_has_entered_info = all(
    (
        ...
        all_inputs.get(field) != CONFIG['form_fields'][field].get(
            'placeholder'
        )
    )
...
if not user_has_entered_info:
    st.info(
        ...
        "your personalized nutritional targets."
    )
    st.header("Sample Daily Targets for Reference 🎯")
    st.caption(
        "These are example targets. Enter your information in the sidebar "
...
st.caption(
    "**PRINCIPLE 1 & 2**: BMR calculated using the most accurate predictive "
    "formula; TDEE using validated activity multipliers"
)
...
metabolic_metrics = [
    ...
    (
        ...
        f"{targets['tdee']} kcal/day"
    ),
    (
        "Daily Caloric Adjustment",
        f"{targets['daily_caloric_adjustment']:+.0f} kcal"
    ),
    (
        "Estimated Weekly Weight Change",
        f"{targets['estimated_weekly_change_kg']:+.3f} kg"
    )
]
...
st.caption(
    "**PRINCIPLE 3**: Caloric targets scaled to individual metabolic reality "
    "using percentage adjustments"
)
...
st.caption(
    "**PRINCIPLE 4**: Protein set first for goal; fat for hormonal health; "
...
distribution_metrics = [
    (
        "Protein",
        f"{targets['protein_percent']:.1f}%",
        f"{targets['protein_calories']} kcal"
    ),
    (
        "Carbohydrates",
        f"{targets['carb_percent']:.1f}%",
        f"{targets['carb_calories']} kcal"
    ),
    (
        "Fat",
        f"{targets['fat_percent']:.1f}%",
        f"{targets['fat_calories']} kcal"
    ),
    ("", "")
]
...
st.markdown(
    "Choose foods using the buttons for preset servings or enter a custom "
    "serving amount for each item."
)
...
if st.button("Calculate Daily Intake", type="primary", use_container_width=True):
    ...
    if selected_foods:
        ...
        for i, item in enumerate(selected_foods):
            with cols[i % 3]:
                st.write(
                    f"• {item['food'].get('emoji', '')} "
                    f"{item['food']['name']} × {item['servings']:.1f}"
                )
    else:
    ...
    # ------ PRINCIPLE 5: Dynamic Monitoring - Progress Tracking ------
    st.markdown("### **PRINCIPLE 5**: Dynamic Monitoring System")
    st.caption(
        "**Scientific Rationale**: Progress tracking encourages monitoring of "
        "actual results vs. predicted outcomes"
    )
    ...
    if recommendations:
        ...
        st.success(
            "🎉 All daily nutritional targets have been met! Excellent work "
            "following your evidence-based plan."
        )
    ...
    current_goal_label = goal_labels.get(
        final_values['goal'], 'your goal'
    )
    ...
    if final_values['goal'] == 'weight_loss':
        ...
        if cal_balance <= -target_deficit * 0.8:  # Within 80% of target deficit
            st.success(
                f"✅ You are in an appropriate caloric deficit of "
                f"{abs(cal_balance):.0f} kcal, supporting "
                f"{current_goal_label}."
            )
        elif cal_balance > 0:
            st.warning(
                f"⚠️ You are consuming {cal_balance:.0f} kcal above "
                f"maintenance. Consider reducing intake for "
                f"{current_goal_label}."
            )
        else:
            st.info(
                f"📊 You have a {abs(cal_balance):.0f} kcal deficit. Target "
                f"deficit for optimal {current_goal_label} is "
                f"{target_deficit:.0f} kcal."
            )

    elif final_values['goal'] == 'maintenance':
        if abs(cal_balance) <= 100:  # Within 100 kcal of maintenance
            st.success(
                f"✅ You are consuming near maintenance calories "
                f"({cal_balance:+.0f} kcal), perfect for "
                f"{current_goal_label}."
            )
        else:
            direction = "above" if cal_balance > 0 else "below"
            st.info(
                f"📊 You are consuming {abs(cal_balance):.0f} kcal "
                f"{direction} maintenance. Adjust intake for precise "
                f"{current_goal_label}."
            )
        ...
        if cal_balance >= target_surplus * 0.8:  # Within 80% of target surplus
            st.success(
                f"✅ You are in an appropriate caloric surplus of "
                f"{cal_balance:.0f} kcal, supporting {current_goal_label}."
            )
        elif cal_balance < 0:
            st.warning(
                f"⚠️ You are consuming {abs(cal_balance):.0f} kcal below "
                f"maintenance. Increase intake for {current_goal_label}."
            )
        else:
            st.info(
                f"📊 You have a {cal_balance:.0f} kcal surplus. Target "
                f"surplus for optimal {current_goal_label} is "
                f"{target_surplus:.0f} kcal."
            )
    ...
    if selected_foods:
        ...
        food_log_data = [{
            'Food Item Name': (
                f"{item['food'].get('emoji', '')} {item['food']['name']}"
            ),
...
st.markdown("""
**Scientific Rationale**: Nutrition provides the building materials, but
**resistance training provides the stimulus** that tells your body what to do
with them. It is a powerful nutrient-partitioning agent.

**Key Benefits by Goal**:
- **During Fat Loss**: Signals the body to preserve precious, metabolically
  active muscle tissue
- **During Weight Gain**: Non-negotiable trigger for muscle growth (surplus
  without training = primarily fat gain)
...
with fitness_cols[1]:
    ...
    current_advice = goal_training_advice.get(
        final_values['goal'], goal_training_advice['weight_gain']
    )
...
st.sidebar.markdown(
    "#### **PRINCIPLE 2**: Activity Level Guide for Accurate TDEE"
)
st.sidebar.markdown("""
**Scientific Rationale**: Accurate activity assessment is crucial for TDEE
calculation.
...
*All recommendations based on peer-reviewed research and professional
guidelines.*
""")

st.sidebar.markdown("---")
st.sidebar.markdown(
    "*Built on evidence-based nutrition science for optimal results.*"
)

# ------ Final Thank You Message ------
print(
    "Thanks for using the Evidence-Based Nutrition Tracker! Remember, "
    "consistency beats perfection every time. Keep tracking, keep training, "
    "and let science guide your journey to better health! 🌟"
)
